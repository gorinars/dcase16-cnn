...
class Dataset(object):
    ...
    def dev(self, fold=0):
        ...
        if fold not in self.evaluation_data_dev:
            self.evaluation_data_dev[fold] = []
            if fold > 0:
                with open(os.path.join(self.evaluation_setup_path, 'fold' + str(fold) + '_evaluate.txt'), 'rt') as f:
                    for row in csv.reader(f, delimiter='\t'):
                        if len(row) == 2:
                            # Scene meta
                            self.evaluation_data_dev[fold].append({
                                'file': self.relative_to_absolute_path(row[0]),
                                'scene_label': row[1]
                            })
                        elif len(row) == 4:
                ...
                self.evaluation_data_dev[0] = data

        return self.evaluation_data_dev[fold]
...
class TUTSoundEvents_2016_DevelopmentSet(Dataset):
    ...
    def dev(self, fold=0, scene_label=None):
        if fold not in self.evaluation_data_dev:
            self.evaluation_data_dev[fold] = {}
            for scene_label_ in self.scene_labels:
                if scene_label_ not in self.evaluation_data_dev[fold]:
                    self.evaluation_data_dev[fold][scene_label_] = []

                if fold > 0:
                    with open(os.path.join(self.evaluation_setup_path, scene_label_+'_fold' + str(fold) + '_evaluate.txt'), 'rt') as f:
                        for row in csv.reader(f, delimiter='\t'):
                            if len(row) == 5:
                                # Event meta
                                self.evaluation_data_dev[fold][scene_label_].append({
                                    'file': self.relative_to_absolute_path(row[0]),
                                    'scene_label': row[1],
                                    'event_onset': float(row[2]),
                                    'event_offset': float(row[3]),
                    ...
                    self.evaluation_data_dev[0][scene_label_] = data

        if scene_label:
            return self.evaluation_data_dev[fold][scene_label]
        else:
            data = []
            for scene_label_ in self.scene_labels:
                for item in self.evaluation_data_dev[fold][scene_label_]:
                    data.append(item)
            return data
